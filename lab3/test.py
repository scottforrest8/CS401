...
def runtest(test):
    try:
        srcname = './'+test.split('_')[1]+'.js'
        inputname = './tests/' + test.split('_')[2] + '.txt'
        subprocess.check_output('node '+srcname+' < '+inputname+' > out.txt', shell=True, timeout=15)
        innumbers = readnumbers(inputname)
        outnumbers = readnumbers('./out.txt')
        subprocess.call('rm ./out.txt', shell=True)
        if srcname == "./sort.js" and list(sorted(innumbers)) == outnumbers:
            return True
        elif srcname == "./reverse.js" and list(reversed(innumbers)) == outnumbers:
            return True
        else:
