...
public:  	
    ...
    class Iter  	
        : public IArray<T>::Iter  //Iter inherents IArray's Iter class which means to make those functions
    {                               //since it is an interface
        ...
        public:
            Iter(AArray<T>& outerarr)
                : outarr(outerarr), size(outarr.size()), pos(0)
                {
                }
